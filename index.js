...
limitawal = 30
memberlimit = 30
cr = '*HENRIQUE BOTS BOT OFC DO WHATSAPP*'
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["5527998401752@s.whatsapp.net","5527998401752@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `iae menbro novo  @${num.split('@')[0]}\seja bem vindo a esse maravilhoso grupo *${mdata.subject}* seja bem vindo leia as regras pfv NOME?  IDADE? MENINA ou MENINO?  NUMERO DO CARTÃO DE CRÉDITO?`
				...
				teks = `PORRA MENO PERDEMOS MAIS UM MEMBRO NESSA POHA DE GRUPO  @${num.split('@')[0]} O PRÓXIMO QUE SAIR MAMA O ADM CLEITIN FDS`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
        ...
        if (levelRole <= 3) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
                ...
                let leaderboarduang = '-----[ *MONEY LEADERBOARD* ]----\n\n'
                let nom = 0
                try {
                    for (let i = 0; i < 10; i++) {
                        nom++
            ...
            break
                ...
                addKoinUser('5527998401752@s.whatsapp.net', fee)
				...
				case 'limit':
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   const aruga1 = q.substring(0, q.indexOf('|') - 0)
					...
					client.sendMessage(from, aruga, image, {caption: 'gostou?', quoted: mek})
					await limitAdd(sender)
					break 
					case 'summer':
					if (!isRegistered) return reply(ind.noregis())
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('mensagens enviadas')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 HENRIQUE BOTS 」*\n\n${body.slice(4)}`)
						}
						reply('mensagens enviadas ')
					}
					break
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Este é o número do meu proprietário faz plaquinha para ele ai ele e mo solitário> _ <, não envie spam ou bloqueie você',MessageType.text, { quoted: mek} )
					break    
           case 'setna':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
				...
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `𝗟𝗶𝘀𝘁 𝗮𝗱𝗺𝗶𝗻 do 𝗴𝗿𝗼𝘂𝗽 *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
